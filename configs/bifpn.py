...
class BIFPN(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, num_outs, start_level=0, end_level=-1, stack=1, add_extra_convs=False,
                 extra_convs_on_inputs=True, relu_before_extra_convs=False, no_norm_on_lateral=False, conv_cfg=None,
        ...
        if end_level == -1:
            ...
            if not num_outs >= self.num_ins - start_level:
        ...
        for ii in range(stack):
            self.stack_bifpn_convs.append(
                BiFPNModule(channels=out_channels, levels=(self.backbone_end_level - self.start_level),
                            conv_cfg=conv_cfg,
                            norm_cfg=norm_cfg,
...
class BiFPNModule(nn.Module):
    ...
    def __init__(self, channels, levels, init=0.5, conv_cfg=None, norm_cfg=None, activation=None, eps=0.0001):
        ...
        for jj in range(2):
            for i in range(self.levels - 1):
                fpn_conv = nn.Sequential(ConvModule(channels,
                                                    channels,
                                                    3,
                                                    padding=1,
                                                    conv_cfg=conv_cfg,
    ...
    def forward(self, inputs):
        ...
        for i in range(levels - 1, 0, -1):
            pathtd[i - 1] = (w1[(0, i - 1)] * pathtd[i - 1] + w1[(1, i - 1)] * F.interpolate((pathtd[i]),
                                                                                             scale_factor=2,
                                                                                             mode='nearest')) / (
        ...
        for i in range(0, levels - 2, 1):
            pathtd[i + 1] = (w2[(0, i)] * pathtd[i + 1] + w2[(1, i)] * F.max_pool2d((pathtd[i]), kernel_size=2) + w2[
                (2, i)] * inputs_clone[i + 1]) / (w2[(0, i)] + w2[(1, i)] + w2[(2, i)] + self.eps)
            pathtd[i + 1] = self.bifpn_convs[idx_bifpn](pathtd[i + 1])
            idx_bifpn = idx_bifpn + 1
