...
def draw_bbox_masks(cfg, dataset, save_dir, split=None, outputs=None, show_score_thr=0):
    ...
    if outputs is None and split is not None:
        ...
        if isinstance(outputs[0], tuple):
            ...
            for idx, output in enumerate(outputs):
                ...
                img_h = dataset.data_infos[idx]['height']
                img_w = dataset.data_infos[idx]['width']
                # bboxes = torch.from_numpy(output[0][:, :4]).to(device='cuda', dtype=torch.float32)
                bbox_results, segm_results = output
                for i, (bbox_result, mask_results) in enumerate(zip(bbox_results, segm_results)):
                    scores = torch.from_numpy(bbox_result[:, 4]).to(device='cuda', dtype=torch.float32)
                    bboxes = torch.from_numpy(bbox_result[:, :4]).to(device='cuda', dtype=torch.float32)
            ...
            for idx, output in enumerate(outputs):
                ...
                for i, bbox_results in enumerate(output):
                    scores = torch.from_numpy(bbox_results[:, 4]).to(device='cuda', dtype=torch.float32)
                    bboxes = torch.from_numpy(bbox_results[:, :4]).to(device='cuda', dtype=torch.float32)
                    labels = [class_names[i]] * bboxes.shape[0]
                    bbox_colors = [(240, 10, 10)] * bboxes.shape[0]
